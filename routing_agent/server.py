...
async def lifespan(app: FastAPI):
    ...
    routing_agent = await RoutingAgent.create([
        f"http://{os.environ["SERVER_URL"]}:{os.environ["COMPUTE_AGENT_PORT"]}",
        f"http://{os.environ["SERVER_URL"]}:{os.environ["STORAGE_AGENT_PORT"]}",
        f"http://{os.environ["SERVER_URL"]}:{os.environ["DATABASE_AGENT_PORT"]}",
        f"http://{os.environ["SERVER_URL"]}:{os.environ["NETWORK_AGENT_PORT"]}",
        f"http://{os.environ["SERVER_URL"]}:{os.environ["COST_ANALYSIS_AGENT_PORT"]}",
        f"http://{os.environ["SERVER_URL"]}:{os.environ["RECOMMENDATION_AGENT_PORT"]}",
    ])
