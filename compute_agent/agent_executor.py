""" Azure AI Foundry Agent Executor for Azure Compute (VM) Optimization """
...
class FoundryAgentExecutor(AgentExecutor):
    ...
    def __init__(self, card: AgentCard):
        self._card = card
        self._foundry_agent: ComputeAgent | None = None

    async def _get_or_create_agent(self) -> ComputeAgent:
        if not self._foundry_agent:
            self._foundry_agent = await create_foundry_compute_agent()
    ...
    async def _process_request(self, message_parts: list[Part], context_id: str, task_updater: TaskUpdater) -> None:
        # Process a user request through the Compute Agent

        try:
            # Retrieve message from A2A parts
            user_message = message_parts[0].root.text

            # Get the compute agent
            agent = await self._get_or_create_agent()

            # Update the task status
            await task_updater.update_status(
                TaskState.working,
                message=new_agent_text_message('Compute Agent is analyzing your VMs...', context_id=context_id)
            )
            ...
            final_message = responses[-1] if responses else 'VM analysis completed.'
            await task_updater.complete(
                message=new_agent_text_message(final_message, context_id=context_id)
            )

        except Exception as e:
            logger.error(f'Compute Agent: Error processing request - {e}', exc_info=True)
            await task_updater.failed(
                message=new_agent_text_message("Compute Agent failed to process the request.",
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        logger.info(f'Compute Agent: Cancelling execution for context {context.context_id}')
