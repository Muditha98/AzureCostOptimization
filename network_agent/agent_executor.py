""" Azure AI Foundry Agent Executor for Azure Network Optimization """
...
from network_agent.agent import NetworkAgent, create_foundry_network_agent
...
class FoundryAgentExecutor(AgentExecutor):
    ...
    def __init__(self, card: AgentCard):
        self._card = card
        self._foundry_agent: NetworkAgent | None = None

    async def _get_or_create_agent(self) -> NetworkAgent:
        if not self._foundry_agent:
            self._foundry_agent = await create_foundry_network_agent()
    ...
    async def _process_request(self, message_parts: list[Part], context_id: str, task_updater: TaskUpdater) -> None:
        # Process a user request through the Network Agent
        ...
        try:
            ...
            await task_updater.update_status(
                TaskState.working,
                message=new_agent_text_message('Network Agent is analyzing your network resources...', context_id=context_id)
            )
            ...
            final_message = responses[-1] if responses else 'Network analysis completed.'
            await task_updater.complete(
                message=new_agent_text_message(final_message, context_id=context_id)
            )

        except Exception as e:
            logger.error(f'Network Agent: Error processing request - {e}', exc_info=True)
            await task_updater.failed(
                message=new_agent_text_message("Network Agent failed to process the request.",
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue):
        logger.info(f'Network Agent: Cancelling execution for context {context.context_id}')
